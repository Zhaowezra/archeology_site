		...
		function renderDocumentTable(topic, modelnum){
			...
			var drows = dtb.selectAll("tr")
						...
						.on("click", function(d){
							console.log(docs[d.index]);
							renderDocumentText(docs[d.index])
							console.log(d)
						})
		...
		async function renderDocumentText(id){
			...
			var newname = id.score.replace(/\//g, "_").replace(":", ".");
			var docdiv = d3.select("#single-doc-div")
			docdiv.select("#doc-title").remove()
			docdiv.select("#doc-content").remove()
			console.log(id)
			...
			docdiv.append("h6").attr("id", "doc-title").text("")
			...
			docp.selectAll('span.words').data(doc_obj.replace(/[\<\>']+/g,'').split(' '))
				.enter()
				.append("span")
